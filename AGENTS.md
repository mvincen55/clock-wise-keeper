...
- **The app is invite-only.** `useAuth` runs an `is_allowed_user()` RPC on every session and immediately signs out anyone not in the `allowed_users` table. There is no public sign-up — the `/auth` page is sign-in only. Reaching authenticated routes (Dashboard, etc.) end-to-end requires an email seeded into `allowed_users` plus an active `org_members` row. A working, allowlisted test account is available in this environment via the secrets **`PURPLE_ENVELOPE_TEST_EMAIL`** and **`PURPLE_ENVELOPE_TEST_PASSWORD`** — sign in with those on `/auth` to land on the authenticated dashboard. (Without credentials, the sign-in form still exercises the live Supabase auth backend — invalid credentials return an "Invalid login credentials" toast — which alone verifies the frontend↔backend integration.)
...
- **Email (and the AI gateway) are managed by Lovable, not by anything you run locally.** Email is sent through Lovable's infra (React Email templates → `enqueue_email` RPC → pgmq queues → the `process-email-queue` cron dispatcher; see `README.md` → "The email system"). You cannot exercise real email sending from a cloud VM. The Lovable MCP server (used to manage the Lovable side, including email) is configured in `.cursor/mcp.json`. It is an HTTP MCP at `https://mcp.lovable.dev/` that authenticates via OAuth — a **cloud agent cannot complete that sign-in**, so its tools are only usable from Cursor Desktop after the user authenticates the server there.
