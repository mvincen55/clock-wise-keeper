...
type PayrollPtoRow={id:string;period_start:string;period_end:string;check_date:string;pto_hours:number;pto_ytd_hours:number|null;worked_hours:number|null;source_file:string;source_page:number;review_note:string|null};
export default function PayrollPtoHistory({employeeId}:{employeeId:string}) {
 ...
 if(!data?.length)return <p className="text-xs text-muted-foreground">No payroll PTO evidence has been matched to this employee yet. Usage has not been verified against payroll.</p>;
 return <section className="space-y-3 border-t pt-4"><h3 className="font-semibold">Payroll-confirmed PTO</h3>
  <p className="text-xs text-muted-foreground">These are pay-period totals, not individual absence dates. Completed payroll weeks use these PTO totals once in the balance; daily entries for the same week are not deducted again. Year-to-date totals are reference only. Periods before the starting balance are historical and are not deducted again.</p>
  <div className="space-y-3">{data.map(r=><div key={r.id} className="rounded border p-3 text-sm">
   ...
   {r.review_note&&<p className="mt-1 text-xs text-amber-700 dark:text-amber-400">{r.review_note}</p>}
  </div>)}</div>
 </section>;
}
