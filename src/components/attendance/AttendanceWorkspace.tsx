...
/**
 * Which hat the page wears. Workplace is everyone's own space, so its
 * Attendance page is personal for managers too; Management holds the
 * whole team. The route decides, never a toggle inside the page.
 */
export type AttendanceMode = 'personal' | 'team';
...
export default function AttendanceWorkspace({ mode }: { mode: AttendanceMode }) {
  const { user } = useAuth();
  ...
  const nameOf = (employeeId: string | null | undefined): string => {
    ...
    return m ? formatEmployeeNameLastFirst(m.displayName) : 'Unknown';
  };
  ...
  const handleAdd = async () => {
    ...
    const target = !personal ? staffById.get(form.employee_id) : undefined;
    if (!personal && !target) return;
    try {
  ...
  const handleRecompute = async () => {
    ...
    if (personal) {
      ...
      if (!focusedMember.userId) {
        toast({ title: 'No login yet', description: `${formatEmployeeNameLastFirst(focusedMember.displayName)} has no linked account, so there is no attendance to recompute. Invite them from the Team page first.`, variant: 'destructive' });
        return;
      }
  ...
  const focusedName = focusedMember ? formatEmployeeNameLastFirst(focusedMember.displayName) : null;
        ...
        <div>
          ...
          {personal && isManager && (
            <Link to={TEAM_ATTENDANCE_PATH} className="mt-1 inline-flex items-center gap-1 text-sm text-primary hover:underline">
              <Users className="h-3.5 w-3.5" /> Team Attendance is under Management
            </Link>
          )}
        </div>
          ...
          {isManager && <Dialog open={open} onOpenChange={openAddDayOff}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Button onClick={handleAdd} disabled={addDayOff.isPending || !addDayOff.isReady || (!personal && !form.employee_id) || (formNotesRequired && !form.notes.trim())} className="w-full">
