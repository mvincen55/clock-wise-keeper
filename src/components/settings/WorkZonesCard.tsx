...
/**
 * Work Zones live inside Office Settings: geofenced clock-in zones are
 * office infrastructure, configured next to closures and payroll. The
 * Settings page gates this to owners and managers.
 */
export default function WorkZonesCard() {
      ...
      <CardHeader className="border-b">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">GPS-based auto clock locations: the office and any satellite addresses.</p>
          </div>
            ...
            <Card key={zone.id} className={`card-elevated ${!zone.is_active ? 'opacity-60' : ''}`}>
              <CardContent className="p-4 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setEditingZone(zone)} aria-label={`Edit ${zone.zone_name}`}>
                    <Save className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => handleDelete(zone.id)} aria-label={`Delete ${zone.zone_name}`}>
