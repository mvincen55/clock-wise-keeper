...
vi.mock('@/hooks/useOrgContext', () => ({ useOrgContext: () => ({ data: { org_id: 'office', employee_id: 'one' } }) }));
vi.mock('@/hooks/useAuth', () => ({ useAuth: () => ({ user: { id: 'login' } }) }));
vi.mock('@/integrations/supabase/client', () => ({ supabase: { from: () => {
  let rows = [
    { org_id: 'office', id: 'overlapping-leave', employee_id: 'one', date_start: '2026-08-29', date_end: '2026-09-03' },
    { org_id: 'office', id: 'earlier-leave', employee_id: 'one', date_start: '2026-08-25', date_end: '2026-08-28' },
    { org_id: 'office', id: 'someone-else', employee_id: 'two', date_start: '2026-09-01', date_end: '2026-09-02' },
  ];
  const query = {
    select: () => query,
...
describe('employee days off', () => {
 ...
 it('keeps personal leave scoped to the logged-in employee even for managers', async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const { result } = renderHook(() => useDaysOff(2026), {
      wrapper: ({ children }) => <QueryClientProvider client={client}>{children}</QueryClientProvider>,
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data?.map(row => row.id)).toEqual(['overlapping-leave', 'earlier-leave']);
    client.clear();
  });
