...
export type MyOperationalRoles = {
  ...
  /**
   * Secondary roles whose assignment window covers today — these are the ones
   * the person is actually covering right now, so their time-sensitive items
   * may be elevated into Today/Attention.
   */
...
export function useMyOperationalRoles(): MyOperationalRoles {
  ...
  return useMemo(() => {
    ...
    // A row with a date window only counts as "covering today" inside it.
    const covers = (row: { starts_on?: string | null; ends_on?: string | null }) =>
      (!row.starts_on || row.starts_on <= today) && (!row.ends_on || row.ends_on >= today);
    ...
    const secondary = secondaryRows.map(r => r.operational_role as OperationalRole);
    const coveringToday = secondaryRows
      .filter(r => covers(r as { starts_on?: string | null; ends_on?: string | null }))
