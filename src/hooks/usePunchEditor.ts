  ...
  return useMutation({
    mutationFn: async ({
      ...
      for (const id of deletedIds) {
        ...
        await insertAudit('punch_deleted', {
          entity_type: 'punch', entity_id: id, field_changed: 'punch',
          old_value: orig?.punch_time || '', new_value: '(deleted)', reason_comment: reason,
        }, entryId, entryDate);
        await supabase.from('punches').delete().eq('id', id);
      }

      // 2. Update edited punches
      const updatedPunches = editedPunches.filter(ep => ep.id && !ep.is_new && !ep.is_deleted && ep.is_edited);
      ...
      const { data: sortedPunches } = await supabase.from('punches')
        .select('id, punch_time').eq('time_entry_id', entryId).order('punch_time', { ascending: true });
      if (sortedPunches) {
        for (let i = 0; i < sortedPunches.length; i++) {
          await supabase.from('punches').update({ seq: i }).eq('id', sortedPunches[i].id);
        }
      }
    },
    onSuccess: () => {
