...
import { useOfficeClosures, useGenerateClosures, useAddClosure, useDeleteClosure, OfficeClosureRow } from '@/hooks/useOfficeClosures';
...
import { Loader2, Settings as SettingsIcon, Shield, Timer, CalendarDays, Plus, Trash2, DollarSign, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className={`flex flex-wrap items-center gap-4 px-4 py-3 ${!row.enabled ? 'opacity-50' : ''}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <Input type="time" value={row.start_time?.slice(0, 5)} onChange={e => onUpdate(row.id, { start_time: e.target.value })} disabled={!row.enabled} className="w-28 text-sm" />
      </div>
      <div className="flex items-center gap-2">
        <Label className="text-xs text-muted-foreground">End</Label>
        <Input type="time" value={row.end_time?.slice(0, 5)} onChange={e => onUpdate(row.id, { end_time: e.target.value })} disabled={!row.enabled} className="w-28 text-sm" />
      </div>
      <div className="flex items-center gap-2">
        <Label className="text-xs text-muted-foreground">Grace (min)</Label>
        <Input type="number" min={0} value={row.grace_minutes} onChange={e => onUpdate(row.id, { grace_minutes: parseInt(e.target.value) || 0 })} disabled={!row.enabled} className="w-20 text-sm" />
      </div>
      <div className="flex items-center gap-2">
        <Label className="text-xs text-muted-foreground">Threshold (min)</Label>
        <Input type="number" min={1} value={row.threshold_minutes} onChange={e => onUpdate(row.id, { threshold_minutes: parseInt(e.target.value) || 1 })} disabled={!row.enabled} className="w-20 text-sm" />
      </div>
    </div>
  );
}
