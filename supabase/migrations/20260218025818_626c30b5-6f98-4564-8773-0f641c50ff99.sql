...
-- 1. Create canonical timezone resolution function
...
-- 2. Create canonical local punch time function
CREATE OR REPLACE FUNCTION public.get_local_punch_time(p_punch_time timestamptz, p_user_id uuid)
  RETURNS timestamp
  LANGUAGE sql
  STABLE SECURITY DEFINER
  SET search_path TO 'public'
AS $$
  SELECT p_punch_time AT TIME ZONE public.get_user_timezone(p_user_id);
$$;

-- 3. Rebuild recompute_attendance_range with fixed logic
  ...
  WHILE cur_date <= p_end_date LOOP
    ...
    IF v_entry.id IS NOT NULL THEN
      ...
      IF v_has_punches THEN
        ...
        -- KEY FIX: Get earliest IN punch where LOCAL DATE matches entry_date
        SELECT p.punch_time, 
        ...
        -- Fallback: if no punch maps to this local date, use earliest IN punch
    ...
    -- 6. Timezone suspect detection (RELAXED):
    --    Only suspect if local time is outside 3am-11pm AND differs from schedule start by > 8 hours
    IF v_first_in_local_time IS NOT NULL AND v_sched_start IS NOT NULL THEN
      IF (v_first_in_local_time < '03:00:00'::time OR v_first_in_local_time > '23:00:00'::time) THEN
        -- Check if it differs from expected start by > 8 hours
