-- Intelligent Sprint Builder: sprints can now target an operational role
-- (front desk, hygienists, …) — the axis the rest of the app already models —
-- alongside the existing team/department/individual scopes. A nullable
-- category records what kind of behaviour an AI-built sprint was about, so
-- future recommendations can learn from real outcomes instead of starting
...
-- The operational roles the signed-in member currently holds. Coverage
-- windows count only while they are open — a stint that ended is not a role.
