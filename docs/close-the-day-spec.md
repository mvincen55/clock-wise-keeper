...
Provider suggestions now show their evidence and narrow instead of guessing. The reader matches codes against the office's own vocabulary — registry schedule codes and codes confirmed in earlier calibrations — so an OCR slip (HY1G for HY16, a dropped digit, "DR" and "02" split into two words) resolves to the known code when the reading recurs, while a clean code that merely differs from a known one (DR03 beside DR02) stays a separate provider. Each column card states what was read (`HY16 read in 6 appointments`) and why that points at a provider (`HY16 is Molly's schedule code`, `DR02 was Dr. Scott in the last calibration`). A code nobody owns is not left blank: its prefix says which kind of provider to look among, and the only doctor or hygienist without a code is offered as a one-click pick with that reason; a lane with a clear majority code offers the majority provider the same way; a lane with no readable code offers the providers not placed anywhere yet, and in calibration whoever held that position last time. Picks are never prefilled — only a registry or previously confirmed code fills a column in, marked "Suggested — confirm". A summary line counts the columns still needing a pick and names the active providers no column has claimed, so a missed provider is visible at a glance. Both calibration and the daily review number the lanes on the preview and shade the lane whose card is hovered or focused. Side-by-side half-width bookings inside one operatory no longer surface as extra columns. Saving a calibration records each newly confirmed code on `org_providers.schedule_code` (with a caution when a DR code lands on a hygienist, or vice versa), so the daily capture recognizes it without a saved layout; codes claimed by two providers or providers read under two codes are left for Settings. Suggestion evidence travels with a column only into review and is stripped before metrics or the saved layout.
